...
async function addToWhitelist(
    ...
    for (let i = 0; i < totalBatches; i++) {
        ...
        const addressesToSet = batch.map(
            (addressesWithTier: AddressWithTier) => addressesWithTier.address
        );
        const tiers = batch.map(
            (addressesWithTier: AddressWithTier) =>
                BigInt(addressesWithTier.tier) * 10n ** 6n
        );

        console.log(addressesToSet, tiers);

        try {
            const tx = await contract.addMultipleToWhitelist(
                addressesToSet,
                tiers
            );
...
async function main(): Promise<void> {
    ...
    readWhitelistCSV('./allocations.csv');
    ...
    try {
        ...
        console.error(
            '\n❌ Failed to add addresses to whitelist:',
            error.message
        );
        process.exit(1);
    }
