...
async function main(): Promise<void> {
    console.log('🚀 Starting Investment Contract Sale');
    console.log('=====================================');
    ...
    try {
        ...
        console.log(
            `   Sale Duration: ${saleDuration.toString()} seconds (${Math.floor(Number(saleDuration) / 60)} minutes)`
        );
        if (saleStart > 0) {
            console.log(
            ...
            if (now >= Number(saleStart) && now <= Number(saleEnd)) {
    ...
    try {
        const isActive = await investmentContract.isSaleActive();
        if (isActive) {
            console.log('⚠️  Sale is already active!');
    ...
    try {
        const tx = await investmentContract.connect(owner).startSale();
