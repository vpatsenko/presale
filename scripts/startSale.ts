...
async function main(): Promise<void> {
    ...
    const presale = (await BackroomPresaleFactory.attach(
        BACKROOM_PRESALE_ADDRESS
    )) as BackroomPresale;
    ...
    console.log(
        '- Total raised:',
        ethers.formatEther(info._totalRaised),
        'ETH'
    );
    ...
    console.log(
        '- Start time:',
        new Date(Number(info._startTime) * 1000).toLocaleString()
    );
    console.log(
        '- End time:',
        new Date(Number(info._endTime) * 1000).toLocaleString()
    );
    console.log(
        '- Time remaining:',
        Number(await presale.getTimeRemaining()),
        'seconds'
    );
}
