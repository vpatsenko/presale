...
async function main(): Promise<void> {
	...
	console.log("Contract address:", PRESALE_ADDRESS);

	console.log("\nConstructor arguments:");
	console.log("- USDC Token Address:", constructorArgs[0]);
	console.log("- Soft Cap:", constructorArgs[1]);
	console.log("- Hard Cap:", constructorArgs[2]);
	console.log("- Min Contribution:", constructorArgs[3]);
	console.log("- Max Contribution:", constructorArgs[4]);
	...
	for (let attempt = 1; attempt <= maxAttempts; attempt++) {
		...
		if (success) {
			...
			if (network.chainId === 8453n) { // Base Mainnet
				...
				console.log(`https://sepolia.basescan.org/address/${PRESALE_ADDRESS}`);
			} else {
				console.log(`Contract address: ${PRESALE_ADDRESS}`);
			}
	...
	if (network.chainId === 84532n) {
		console.log(`   https://sepolia.basescan.org/address/${PRESALE_ADDRESS}#code`);
	} else {
		console.log(`   https://basescan.org/address/${PRESALE_ADDRESS}#code`);
	}
