...
const TestTokenAddress = '0x8b968c0564a7ac79735a7a8bb4a94fe3602d3522';
...
async function main(): Promise<void> {
    ...
    if (!TestTokenAddress) {
        console.error('❌ TestTokenAddress not found in environment variables');
        console.log('Please set TestTokenAddress in your .env file');
        process.exit(1);
    }
    ...
    console.log('Contract address:', TestTokenAddress);
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (success) {
            ...
            if (network.chainId === 8453n) {
                // Base Mainnet
                console.log(`https://basescan.org/address/${TestTokenAddress}`);
            } else if (network.chainId === 84532n) {
                // Base Sepolia
                console.log(
                    `https://sepolia.basescan.org/address/${TestTokenAddress}`
                );
            } else {
                console.log(`Contract address: ${TestTokenAddress}`);
            }
    ...
    if (network.chainId === 84532n) {
        console.log(
            `   https://sepolia.basescan.org/address/${TestTokenAddress}#code`
        );
    } else {
        console.log(`   https://basescan.org/address/${TestTokenAddress}#code`);
    }
